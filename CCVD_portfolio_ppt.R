...
PDKE_Short
PDKE_S <- external_img(src = PDKE_Short,
  height = 1,
  width = 1)

PDKE_Long <-
  paste0(I_FOLDER, "PDKE_Logo_Color_Horizontal_Type-05.png")
PDKE_L <- external_img(src = PDKE_Long,
  height = 1,
  width = 1)

EWCfile <- paste0(I_FOLDER, "EWC No Boarder.png")
EWCimg <- external_img(src = EWCfile,
  height = 1,
  width = 1)

EWClfile <- paste0(I_FOLDER, "ewc_long.png")
EWClimg <- external_img(src = EWClfile,
  height = 1,
  width = 1)

CASCfile <- paste0(I_FOLDER, "PICASC_NoBoarder.jpg")
CASCimg <- external_img(src = CASCfile ,
  height = 1,
  width = 1)

FSfile <- paste0(I_FOLDER, "FS.png")
FSimg <- external_img(src = FSfile,
  height = 1,
  width = 1)

UHfile <- paste0(I_FOLDER, "UH.png")
UHimg <- external_img(src = UHfile,
  height = 1,
  width = 1)

USGSfile <- paste0(I_FOLDER, "USGS.png")
USGSimg <- external_img(src = USGSfile,
  height = 1,
  width = 1)
...
NIDISimg <- external_img(src = NIDISfile,
  height = 1,
  width = 1)

NOAAfile <- paste0(I_FOLDER, "NOAA.png")
NOAAimg <- external_img(src = NOAAfile,
  height = 1,
  width = 1)

HAVOfile <- paste0(I_FOLDER, "HAVO.png")
HAVOimg <- external_img(src = HAVOfile,
  height = 1,
  width = 1)
...
DOFAWimg <- external_img(src = DOFAWfile,
  height = 1,
  width = 1)

DLNRfile <- paste0(I_FOLDER, "DLNR.jpg")
DLNRimg <- external_img(src = DLNRfile,
  height = 1,
  width = 1)

DPlanfile <- paste0(I_FOLDER, "DPlan.png")
DPlanimg <- external_img(src = DPlanfile,
  height = 1,
  width = 1)

PIDPfile <- paste0(I_FOLDER, "PIDP.png")
PIDPimg <- external_img(src = PIDPfile,
  height = 1,
  width = 1)

CLfile <- paste0(I_FOLDER, "Clark.png")
CLimg <- external_img(src = CLfile,
  height = 1,
  width = 1)

PRfile <- paste0(I_FOLDER, "PRISCC logo.png")
PRimg <- external_img(src = PRfile,
  height = 1,
  width = 1)

FLfile <- paste0(I_FOLDER, "funder_logos.png")
FLimg <- external_img(src = FLfile,
  height = 1,
  width = 1)
...
fp_BR3 <-
  fp_text(bold = TRUE,
    color = "darkgreen",
    font.size = 40)
fp_NM <- fp_text(bold = TRUE,
  font.size = 20,
  underlined = TRUE)
fp_NMS <- fp_text(bold = TRUE,
  font.size = 8,
  underlined = TRUE)
...
fp_NM6 <- fp_text(font.size = 18,
  color = "black",
  bold = TRUE)
fp_NM7 <- fp_text(font.size = 18, color = "black")
fp_NM8 <- fp_text(font.size = 16,
  color = "black",
  bold = TRUE)
...
FTXTT <-  fp_text(bold = TRUE,
  color = "darkblue",
  font.size = 40)
FTXTT2 <- fp_text(italic = TRUE,
  color = "black",
  font.size = 40)
FTXTT3 <-
  fp_text(italic = TRUE,
...
TIT <-
  fpar(
    ftext("Climate Change, Climate Variability, & Drought Portfolio", fp_BR),
...
if ((nchar(NameF)) > (nchar(SNameF))) {
  HDKE <-
    paste0(
      "Climate change, climate variability, and drought (CCVD) will exert a growing impact on Hawaii's ecosystems, ",
...
This CCVD portfolio is a comprehensive synthesis of climate and drought information developed specifically for ",
      NameF,
      " (",
      SNameF,
      "). ",
...
p <- p + 1
p2 <- p
S3_TIT <- block_list(fpar(
  ftext("Part 1: Describing the Area", FTXTT),
  fp_p = fp_par(text.align = "center")
))

CCVD <-
  paste0(
    "In describing any area of management in Hawaii, it is important to present both traditional and contemporary knowledge. ",
...
This portfolio provides a brief description of ",
    SNameF,
    " in context of traditional Hawaiian landscapes, as well as contemporary knowledge on ",
...
MP <- block_list(fpar(ftext(
  paste0(
    "Mokupuni is the largest land division and refers to entire islands. ",
    SNameF,
    " is in the mokupuni of ",
    ISL,
    "."
  ),
  fp_Fig5
...
if (nrow(MOKU) == 1) {
  MO <- block_list(fpar(ftext(
    paste0(
      "Within mokupuni are smaller divisions called moku. ",
      SNameF,
      " is in the moku of ",
      MO1,
      "."
...
if (AHc < 3) {
  AH <- block_list(fpar(ftext(
    paste0(
      "Within each moku are several ahupuaa which commonly extend from uplands to the sea. ",
      SNameF,
      " is situated within ",
      AHc,
      " ahupuaa -  ",
...
ft1 <-
  block_list(fpar(ftext(
    paste0("https://planning.hawaii.gov/gis"), fp_ft
  )))
...
S5_TIT <- block_list(fpar(ftext("Elevation", FTXTT), fp_p = fp_par(text.align = "center")))
...
ELEV_T <-
  paste0(
    ...
    ". It covers ",
    ar,
    " acres and a vertical elevation range of ",
    EL_Dif,
    ELUnit,
    ". In Hawaii, climate gradients can change ",
...
M_Elev <-  block_list(
  ...
  fpar(ftext("          ", fp_NMS)),
  fpar(ftext(
    paste0("Minimum = ", El_Min, ELUnit), fp_NM3
  )),
  fpar(ftext(paste0(
    "Mean = ", El_Mean, ELUnit
  ), fp_NM3)),
  fpar(ftext(
    paste0("Maximum = ", El_Max, ELUnit), fp_NM3
  ))
)

# Figure 2 caption
FIG_2 <- block_list(fpar(ftext(
  paste0(
    "Figure 2. Elevation for the Island of ",
    ISL,
    " with ",
    SNameF,
    " outlined in black."
...
ft2 <-
  block_list(fpar(ftext(
    paste0("https://www.pacioos.hawaii.edu/"), fp_ft
  )))
...
S6.1_TIT <- block_list(fpar(ftext("Landcover", FTXTT), fp_p = fp_par(text.align = "center")))
...
LAND_T <-
  paste0(
    "The three most common types of landcover in ",
    SNameF,
    " are ",
    LC1,
    ", ",
...
# Figure 3.1 caption
FIG_3.1 <- block_list(fpar(ftext(
  paste0(
    "Figure 3. Bar graph showing amount and percent of each landcover type within ",
    SNameF,
    "."
  ),
  fp_Fig
...
# Figure 3.2 caption
FIG_3.2 <- block_list(fpar(ftext(
  paste0(
    "Figure 4. Landcover mapping for the island of ",
    ISL,
    " with ",
    SNameF,
    " outlined in red. ",
    "The maps shown in the following slides will be for the ",
    SNameF,
    " area only."
  ),
  fp_Fig
...
ft3 <-
  block_list(fpar(ftext(
    paste0("https://www.usgs.gov/special-topics/lcmap"), fp_ft
  )))
...
ft3.1 <-
  block_list(fpar(ftext(
    paste0("https://geoportal.hawaii.gov/datasets"), fp_ft
  )))
...
# Figure caption
FIG_6.b <- block_list(fpar(ftext(
  paste0(
    "Figure 6. Streams and hydrologic features with ",
    SNameF,
    " outlined in red."
  ),
  fp_Fig
...
P1_TIT <- block_list(fpar(ftext("Part 2: Climate Characteristics", FTXTT)),
  fpar(ftext(SNameF, FTXTT3), fp_p = fp_par(text.align = "center")))

PART1 <-
  paste0(
    "In developing this Portfolio, we relied on several gridded climate products available for the State of Hawaii. Annual and monthly estimates of rainfall were ",
    ...
    "We retrieved all the data points that fell within the boundaries of ",
    SNameF,
    " from our 250 meter resolution state-wide maps to support the presented analyses."
  )
...
S8.0_TIT <- block_list(fpar(
  ftext("Rainfall Station Locations", FTXTT),
  fp_p = fp_par(text.align = "center")
))
...
RS1 <-
  paste0(
    "Rainfall data for this portfolio were estimated based on measurements made ",
...
FIG_5.a <- block_list(fpar(ftext(
  paste0(
    "Figure 6. Rainfall station locations across ",
    ISL2,
    " with ",
    SNameF,
...
# Figure 6 caption
FIG_6.a <- block_list(fpar(ftext(
  paste0(
    "Figure 7. Three closest stations to ",
    SNameF,
    " with links to more ",
...
S6_TIT <- block_list(fpar(
  ftext("Annual Climate Characteristics", FTXTT),
  fp_p = fp_par(text.align = "center")
))

ANNCLIM_T <-
  paste0(
    "Climatic conditions in Hawaii can vary greatly across the landscape. ",
    "These maps show the variation in select climate variables across ",
    SNameF,
    " and the table below has min. and max. values taken from the maps."
  )
M_ANNCLIM <-  block_list(fpar(ftext(ANNCLIM_T, fp_Tx)))
...
FIG_5.1 <- block_list(fpar(ftext(
  paste0(
    "Table 2. Minimum and maximum average annual values for selected climate variables from within ",
    SNameF,
    "."
  ),
  fp_Fig
...
FIG_6.1 <- block_list(fpar(ftext(
  paste0(
    "Figure 8. Mean annual climate of ",
    SNameF,
...
S7_TIT <- block_list(fpar(
  ftext("Average Monthly Rainfall", FTXTT),
  fp_p = fp_par(text.align = "center")
))
...
CLIMO_RF <-
  paste0(
    "Average monthly rainfall patterns vary over the course of the year. At ",
    SNameF,
    ", the highest monthly rainfall is received in ",
    MONLIST[RFxm],
    " (",
    RFx,
    RFUnit,
    ".) and the lowest monthly rainfall is received in ",
    MONLIST[RFnm],
    " (",
    RFn,
    RFUnit,
...
FIG_6a <- block_list(fpar(ftext(
  paste0("Figure 9. Mean monthly rainfall at ", SNameF, "."), fp_Fig
)))

FIG_7a <- block_list(fpar(ftext(
  paste0(
...
S8a_TIT <- block_list(fpar(
  ftext("Average Monthly Temperature", FTXTT),
  fp_p = fp_par(text.align = "center")
))
...
CLIMO_TA <-
  paste0(
    "Average monthly air temperature patterns vary over the course of the year. At ",
    SNameF,
    " there is a ",
    TAv,
    TUnit2,
    " annual variation in temperature,",
    " with the warmest month of ",
    MONLIST[TAxm],
    " (",
    TAx,
    TUnit2,
    ") and the coolest month in ",
    MONLIST[TAnm],
    " (",
    TAn,
    TUnit2,
...
FIG_8a <- block_list(fpar(ftext(
  paste0(
    "Figure 11. Mean monthly air temperature at ",
    SNameF,
    " with monthly rainfall in the background."
  ),
  fp_Fig
)))

FIG_9a <- block_list(fpar(ftext(
  paste0(
...
S8_TIT <- block_list(fpar(
  ftext("Average Monthly Climate", FTXTT),
  fp_p = fp_par(text.align = "center")
))
...
FIG_6 <- block_list(fpar(ftext(
  paste0(
    "Figure 14. Mean monthly temperature at ",
    SNameF,
    " with area average shown in heading of each plot."
  ),
  fp_Fig
...
S9_TIT <- block_list(fpar(
  ftext("Average Seasonal Rainfall", FTXTT),
  fp_p = fp_par(text.align = "center")
))
...
SEA <-
  paste0(
    "Hawaii has two distinct 6-month seasons of rainfall: hooilo (Wet season: November to April) and kau (Dry season: May to October).",
    " Average Wet season monthly rainfall across ",
    SNameF,
    " is ",
    WetM,
    " ",
    ...
    " ",
    RFUnit2,
    ". These monthly values are in the ",
    W_RF_P,
    " and ",
    D_RF_P,
...
fp_Tx19 <- fp_text(italic = TRUE,
  color = "black",
  font.size = 19)
fp_SEA <- fpar(ftext(SEA, fp_Tx19))

FIG_12.1 <- block_list(fpar(ftext(
  paste0(
    "Figure 15. Average monthly rainfall maps for the wet (top) and dry (bottom) seasons. ",
    SNameF,
    "."
  ),
  fp_Fig
...
TAB1 <- block_list(fpar(ftext(
  paste0(
    "Table 3. Average monthly climate variables characteristics at ",
    NameF,
    ". Where, RF is rainfall; Min TA is average minimum air temperature",
    ...
    "evapotranspiration; SM is soil moisture; S is shortwave downward radiation: ANN, is annual total for rainfall and annual average for all other variables."
  ),
  fp_Fig
)))
...
P2_TIT <- block_list(fpar(
  ftext("Part 3: Climate Variability", FTXTT),
  fp_p = fp_par(text.align = "center")
))
# fpar(ftext(SNameF, FTXTT3),fp_p = fp_par(text.align = "center")))

fp_Txa <- fp_text(italic = TRUE,
  color = "black",
  font.size = 17)

InterAn <-  block_list(fpar(ftext(
  paste0(
    "Rainfall and temperature in Hawaii can vary greatly from year-to-year due to natural climatic systems such as the El Niño-Southern Oscillation (ENSO). ",
    ...
    "ENSO consists of five phases, as shown in the graph below."
  ),
  fp_Txa
)))
...
MEIimg <- external_img(src = MEIfile,
  height = 1,
  width = 2)

ENSOfile <- paste0(I_FOLDER, "ENSO_New.png")
ENSO2img <- external_img(src = ENSOfile,
  height = 1.2,
  width = 1.2)

FIG_10.1 <- block_list(fpar(ftext(
  paste0(
    "Figure 16. Timeseries of changes in sea surface temperature (SST) and associated ENSO phase from 1950 - 2022. ",
    SNameF,
    "."
  ),
  fp_Fig
...
ft4 <-
  block_list(fpar(ftext(
    paste0("https://origin.cpc.ncep.noaa.gov/"), fp_ft
  )))
...
EN_TIT <- block_list(fpar(
  ftext("Seasonal Rainfall and ENSO", FTXTT),
  fp_p = fp_par(text.align = "center")
))
...
MEI2 <-
  paste0(
    "In Hawaii, the Warm (El Niño) phase typically brings below average rainfall during the wet season, and above average rainfall in the dry season. This pattern is reversed for the Cool (La Niña) phase.

At ",
    SNameF,
    ", the wet season during a Strong El Niño is ",
    DifWRF,
    "% dryer than the long-term wet season average, and the dry season during a Strong La Niña is ",
    DifDRF,
    "% dryer than average. These patterns influence drought conditions and wildfire susceptibility, and management activities can benefit from incorporating this ENSO-influenced seasonal rainfall variability."
  )
MEI3 <-  block_list(fpar(ftext(MEI2, fp_Txa)))

MEISfile <- paste0(PROJECT_FILE_BASE, "ENSO_season_barplot.png")
MEISimg <- external_img(src = MEISfile,
  height = 1,
  width = 2.5)

FIG_14.1 <- block_list(fpar(ftext(
  paste0(
    "Figure 17. Barplot of average monthly rainfall grouped by season and ENSO phase. Numbers above the bars are how many seasons from 1950 to ",
    yr,
...
S13_TIT <- block_list(fpar(
  ftext("Long-Term Trends in Rainfall", FTXTT),
  fp_p = fp_par(text.align = "center")
))

Trend <-
  paste0(
    "Linear trends in annual and seasonal rainfall at ",
    SNameF,
    " have been calculated over three different periods since 1920 ",
...
FIG_9 <- block_list(fpar(ftext(
  paste0(
    "Figure 18. Rainfall time series (1920-",
    yr,
    ") at " ,
    SNameF,
    " with ",
...
S14_TIT <- block_list(fpar(
  ftext("Trends in Air Temperature", FTXTT),
  fp_p = fp_par(text.align = "center")
))
...
if (pred[[1]] > pred[[2]]) {
  airchange <- c("decreased")
}
if (pred[[1]] < pred[[2]]) {
  airchange <- c("increased")
}
...
AirTrend <-
  paste0(
    ...
    " to ",
    e,
    " average annual air temperature has ",
    airchange,
    " by ",
    airchange2
  )
...
FIG_10_11 <- block_list(fpar(ftext(
  paste0(
    "Figure 19. ",
    yrs,
    "-year (",
    ...
    " to ",
    e,
    ") monthly air temperature time series at ",
    SNameF,
...
ft5 <-
  block_list(fpar(ftext(
    paste0("https://www.hawaii.edu/climate-data-portal/"),
    fp_ft
  )))
...
P3_TIT <- block_list(fpar(ftext("Part 4: Drought and Fire History", FTXTT)),
  fpar(ftext(SNameF, FTXTT3), fp_p = fp_par(text.align = "center")))

Part3 <-
  paste0(
    "Drought is a prominent feature of the climate system in Hawaii and can cause severe impacts across multiple sectors. ",
...
TIT_D <-
  fpar(ftext("Five Types of Drought", FTXTT),
    fp_p = fp_par(text.align = "center"))

D_t <-
  paste0(
    "There are five major types of drought. During droughts there is sometimes a progression from ",
...
S15_TIT <- block_list(fpar(
  ftext("Identifying Droughts Using the", FTXTT),
  fp_p = fp_par(text.align = "center")
),
  fpar(
    ftext("Standard Precipitation Index", FTXTT),
    fp_p = fp_par(text.align = "center")
  ))
...
SPI <-
  paste0(
    "The Standardized Precipitation Index (SPI) is one of the most widely used indices for meteorological drought. SPI compares current rainfall with ",
...
S16_TIT <- block_list(fpar(
  ftext("A 100+ Year History of Drought", FTXTT),
  fp_p = fp_par(text.align = "center")
))
...
if (Drot_ct > 12) {
  DROT2 <- DROT[13:nrow(DROT), ]
}
if (Drot_ct > 24) {
  DROT2 <- DROT2[1:12, ]
}
if (Drot_ct > 24) {
  DROT3 <- DROT[25:nrow(DROT), ]
}
...
if (exists("DROT2")) {
  ...
  DrotT_T2 <-
    bg(DrotT_T2, bg = "orange", part = "header")
  
  DrotT_T2 <- autofit(DrotT_T2)
}

if (exists("DROT3")) {
  DrotT_T3 <- flextable(DROT3[1:6])
  
  DrotT_T3 <-
    bg(DrotT_T3, bg = "orange", part = "header")
...
DHist1 <-
  paste0(
    "Negative SPI values (dry periods) are inverted to show a complete drought timeseries at ",
    SNameF,
    ". Dashed lines and corresponding color coding indicates instances ",
...
DHist2 <-
  paste0(
    "A total of ",
    CNTDRT,
    " droughts were observed over the entire record ",
    "with a total of ",
    SoG,
...
fp_DHist <- block_list(fpar(ftext(DHist1, fp_Tx)),
  fpar(ftext(" ", fp_Tx)),
  fpar(ftext(DHist2, fp_Tx)))

DHistfile <- paste0(PROJECT_FILE_BASE, "Drought_History.png")
DHistimg <- external_img(src = DHistfile,
  height = 2,
  width = 4)

FIG_13 <- block_list(fpar(ftext(
  paste0(
    "Figure 21. SPI time series (1920-",
    yr,
    ") (reversed axis) at ",
    SNameF ,
...
SPI3_SMimg <-
  external_img(src = SPI_3_SMfile,
    height = 2,
    width = 4)

SPI_12_SMfile <-
  paste0(PROJECT_FILE_BASE, "Drought_HistoryS_12.png")
SPI12_SMimg <-
  external_img(src = SPI_12_SMfile,
    height = 2,
    width = 4)

FIG_14 <- block_list(fpar(ftext(
  paste0("Figure 22. SPI-3 time series (reversed axis) at ", NameF , "."),
  fp_Fig
)))
FIG_15 <- block_list(fpar(ftext(
  paste0("Figure 23. SPI-12 time series (reversed axis) at ", NameF , "."),
  fp_Fig
)))
SPI3v12 <- block_list(fpar(ftext(
  paste0(
    "SPI-3 provides a comparison of rainfall over a specific 3-month period and reflects short-term ",
    "conditions. SPI-12 provides 12-month comparisions and reflects long-term conditions. It is important ",
    "to consider both timescales for planning."
  ),
  fp_Txa
)))
...
if (spi3HC$P.Intensity <= 2 &&
    spi3HC$P.Intensity > 1.5) {
  spi3HRL <- "severe"
}
if (spi3HC$P.Intensity > 2) {
  spi3HRL <- "extreme"
...
if (!is.na(spi3HC$End)) {
  spi3CT <-
    paste0(
      "SPI-3: Currently not in drought. Most recently there was ",
      spi3HRL,
      " drought from ",
      spi3HRs,
      " - ",
      spi3HRe,
      ". Current drought intensity is ",
      spi3C,
      "."
    )
}
...
if (spi12HC$P.Intensity <= 2 &&
    spi12HC$P.Intensity > 1.5) {
  spi12HRL <- "severe"
}
if (spi12HC$P.Intensity > 2) {
  spi12HRL <- "extreme"
...
if (!is.na(spi12HC$End)) {
  spi12CT <-
    paste0(
      "SPI-12: Currently not in drought. Most recently there was ",
      spi12HRL,
      " drought from ",
      spi12HRs,
      " - ",
...
SPI3v12.1 <- block_list(fpar(ftext(
  paste0(
    ...
    " ",
    RFC.y,
    " the most recent drought events are as follows:"
  ),
  fp_NM8
)))
#SPI3v12.1
...
S18_TIT <- block_list(fpar(ftext(paste(
  "Fire Occurrence in ", ISL
), FTXTT), fp_p = fp_par(text.align = "center")))


FIRE <-
  paste0(
    "Ecological drought often drives an increase in wildfire occurrence. In Hawaii, wildfires are most extensive in dry ",
...
FIG_16 <- block_list(fpar(ftext(
  paste0(
    "Figure 24. The map shows wildfires that have occurred on the island of ",
    ISL,
...
P4_TIT <- block_list(fpar(
  ftext("Part 5: Future Climate", FTXTT),
  fp_p = fp_par(text.align = "center")
),
  fpar(ftext(SNameF, FTXTT3), fp_p = fp_par(text.align = "center")))
...
Part4 <-
  paste0(
    "Global Climate Models are used to predict future changes in rainfall and temperature, simulating future conditions under different ",
...
Part4.2 <-
  paste0(
    "Data downscaling is used make these models useful at the local management level. In Hawaii, two types of downscaled ",
...
Part4.4 <-
  paste0(
    "Both downscaling projections are presented here. These two projections sometimes agree with each other, ",
...
S21_TIT <- block_list(fpar(
  ftext("Average Rainfall Change", FTXTT),
  fp_p = fp_par(text.align = "center")
),
  fpar(
    ftext("Mid-Century (2040-2070)", FTXTT),
    fp_p = fp_par(text.align = "center")
  ))
...
ifelse(RFA_Thresh40_4.5 >= 0,
  RF40_4.5_ch <- "increase",
  RF40_4.5_ch <- "decrease")
ifelse(RFA_Thresh40_8.5 >= 0,
  RF40_8.5_ch <- "increase",
  RF40_8.5_ch <- "decrease")
...
RF_Down4 <-  block_list(
  ...
  fpar(ftext(
    paste0(
      "These Statistical Downscaling maps show the projected change in rainfall under RCP 4.5 and 8.5 ",
      "conditions. At ",
      SNameF,
      ", annual rainfall is projected to ",
      RF40_4.5_ch,
      " by ",
      abs(RF40_4.5),
      " inches (RCP 4.5), ",
...
FIG_18 <- block_list(fpar(ftext(
  paste0(
    "Figure 25. Downscaled future rainfall projections (% change from present) ",
    "at ",
    SNameF,
    " by mid-century (2040-2070) using Statistical Downscaling. RCP 4.5 ",
...
S20_TIT <- block_list(fpar(
  ftext("Average Rainfall Change", FTXTT),
  fp_p = fp_par(text.align = "center")
),
  fpar(
    ftext("End-of-Century (2100)", FTXTT),
    fp_p = fp_par(text.align = "center")
  ))
...
ifelse(RF100_min >= 0,
  RF100_min <- paste0("+", RF100_min),
  RF100_min <- RF100_min)
RF100_min
ifelse(RF100_max >= 0,
  RF100_max <- paste0("+", RF100_max),
  RF100_max <- RF100_max)
...
if (a == b && c == d)
{
  RF2100t <-
    ftext(
      paste0(
        "These Dynamical and Statistical Downscaling maps show the projected ",
        "change in rainfall under RCP 4.5 and 8.5 conditions. At ",
        SNameF,
        ", annual rainfall is projected to ",
        RF100_4.5t,
        " by ",
        ...
        " by ",
        abs(RF100_8.5a),
        " inches (RCP 8.5) by end-of-century."
      ),
      fp_NM2
    )
}
  ...
  RF2100t <-
    ftext(
      paste0(
        ...
        " by ",
        abs(RF100_4.5a),
        " inches (RCP 4.5). The models do not agree on the direction of change for RCP 8.5."
      ),
      fp_NM2
    )
}
  ...
  RF2100t <-
    ftext(
      paste0(
        ...
        "change in rainfall under RCP 4.5 and 8.5 conditions. At ",
        SNameF,
        ", the models do not agree on the overall direction of change for either RCP 4.5 or 8.5."
      ),
      fp_NM2
    )
}
...
RF_Down <-  block_list(
  ...
  fpar(ftext(
    paste0(RF100_4.5a, " to ", RF100_8.5a, RFUnit, "/year"), fp_NM3
  )),
...
FIG_17 <- block_list(fpar(ftext(
  paste0(
    "Figure 26. Downscaled future rainfall projections (% change from present) ",
    "at ",
    SNameF,
    " by end-of-century (2100), using both Dynamical and Statistical ",
...
S23_TIT <- block_list(fpar(
  ftext("Average Air Temperature Change", FTXTT),
  fp_p = fp_par(text.align = "center")
),
  fpar(
    ftext("Mid-Century (2040-2070)", FTXTT),
...
ifelse(TAA_Thresh40_4.5 >= 0,
  TA40_4.5_ch <- "increase",
  TA40_4.5_ch <- "decrease")
ifelse(TAA_Thresh40_8.5 >= 0,
  TA40_8.5_ch <- "increase",
  TA40_8.5_ch <- "decrease")
...
TA_Down4 <-  block_list(
  ...
  fpar(ftext(
    paste0(
      "These Statistical Downscaling maps show the projected change in air temperature under RCP 4.5 and 8.5 ",
      "conditions. At ",
      SNameF,
      ", air temperature is projected to ",
      TA40_4.5_ch,
      " by ",
      abs(TAA_Thresh40_4.5),
      "°F (RCP 4.5), ",
      "or ",
      TA40_8.5_ch,
      " by ",
      abs(TAA_Thresh40_8.5),
      "°F (RCP 8.5) by mid-century."
    ),
    fp_NM2
  ))
)
...
FIG_20 <- block_list(fpar(ftext(
  paste0(
    "Figure 27. Downscaled future temperature projections (°F change from present) ",
    "at ",
    SNameF,
    " by mid-century (2040-2070) using Statistical Downscaling. ",
...
ifelse(
  abs(TAA_Thresh100_D4.5) >= abs(TAA_Thresh100_S4.5),
  TA100_4.5c <- TAA_Thresh100_S4.5,
  TA100_4.5c <- TAA_Thresh100_D4.5
)
ifelse(
  abs(TAA_Thresh100_D8.5) >= abs(TAA_Thresh100_S8.5),
  TA100_8.5c <- TAA_Thresh100_D8.5,
  TA100_8.5c <- TAA_Thresh100_S8.5
)
...
if (a == b && c == d)
{
  TA2100t <-
    ftext(
      paste0(
        "These Dynamical and Statistical Downscaling maps show the projected ",
        "change in air temperature under RCP 4.5 and 8.5 conditions. At ",
        SNameF,
        ", air temperature is projected to ",
        TA100_4.5t,
        " by ",
  ...
  TA2100t <-
    ftext(
      paste0(
        ...
        " by ",
        abs(TA100_4.5c),
        "°F (RCP 4.5). The models do not agree on the direction of change for RCP 8.5."
      ),
      fp_NM2
    )
}
...
TA_Down <-  block_list(
  ...
  fpar(ftext("Change in Air Temperature", fp_NM6)),
  fpar(ftext(
    paste0(TA100_4.5c, " to ", TA100_8.5c, "°F "), fp_NM3
  )),
...
FIG_19 <- block_list(fpar(ftext(
  paste0(
    "Figure 28. Downscaled future rainfall projections (% change from present) ",
    "at ",
    SNameF,
    " by end-of-century (2100), using both Dynamical and Statistical ",
    "downscaling. RCP 4.5 (top row) and RCP 8.5 (bottom row)."
...
S24_TIT <- block_list(fpar(
  ftext("Part 6: CCVD Summary", FTXTT),
  fp_p = fp_par(text.align = "center")
),
  fpar(ftext(SNameF, FTXTT3), fp_p = fp_par(text.align = "center")))
...
Summary <-
  paste0(
    ...
    ELUnit,
    "). ",
    "Rainfall varies over the course of the year with a maximum of ",
    RFx,
    RFUnit3,
    " occurring in ",
    MONLIST[RFxm],
    ...
    " occurring in ",
    MONLIST[RFnm],
    ". On average, wet season months (Nov-Apr) receive ",
    SeaD,
    RFUnit,
    " more rainfall than dry season months (May-Oct). ",
    "Seasonal rainfall can vary within the unit as well, with dry season rainfall ranging from ",
    D_RF_R,
    RFUnit3,
    " and wet season rainfall ",
    "ranging from ",
    W_RF_R,
    RFUnit3,
    " across the ",
    ...
    "average, ",
    DifWRF,
    "% less rainfall is received, relative to the long-term average. ",
    "The average temperature at ",
    SNameF,
    " is ",
    TAA,
    TUnit,
    " but temperature ranges from ",
    TAn,
    TUnit,
    " to ",
    TAx,
    ...
    "Drought is a reoccurring feature in the climate system of ",
    SNameF,
    " with a total of ",
    CNTDRT,
    " occurring over the record which is approximately ",
    DecD,
    " per decade. A total of ",
    SoG,
    " drought events were at severe strength ",
    "or greater and the longest drought lasted for a total of ",
    LongD,
    " consecutive months. ",
    "Future projections of rainfall are uncertain, with end-of-century annual changes ranging from ",
    min(RFA_Thresh100),
    " to " ,
    max(RFA_Thresh100),
    ". Future projections of temperature suggest an increase of ",
    TAA_Thresh40_4.5,
    TUnit,
    " to ",
    TAA_Thresh40_8.5,
    TUnit,
    " by mid century (2040-2070) ",
    "and an increase of ",
    TAA_100m,
    TUnit,
    " to ",
...
S25_TIT <- block_list(fpar(
  ftext("External Resources", FTXTT),
  fp_p = fp_par(text.align = "center")
))

fp_NM3a <-
  fp_text(font.size = 14, color = "darkgreen")
...
fp_RES <-  block_list(
  ...
  fpar(ftext(
    "https://droughtmonitor.unl.edu/", fp_NM3a
  )),
  fpar(ftext("                    ", fp_Fig2)),
  fpar(ftext(
    "NIDIS Current Hawaii Drought Maps"      , fp_NM6a
  )),
  fpar(ftext(
    "https://www.drought.gov/states/hawaii", fp_NM3a
  )),
  fpar(ftext("                    ", fp_Fig2)),
  ...
  fpar(ftext("     "   , fp_Fig2)),
  fpar(ftext(
    "Pacific Drought Knowledge Exchange", fp_NM6a
  )),
  fpar(ftext(
    "http://www.soest.hawaii.edu/pdke/", fp_NM3a
  )),
  fpar(ftext("                    ", fp_Fig2)),
  fpar(ftext("Pacific Fire Exchange", fp_NM6a)),
  fpar(ftext(
    "https://www.pacificfireexchange.org/", fp_NM3a
  )),
  fpar(ftext("                    ", fp_Fig2)),
...
S26_TIT <- block_list(fpar(ftext("Acknowledgements", FTXTT), fp_p = fp_par(text.align = "center")))


Ack <-
  paste0(
    "Ryan Longman, Abby Frazier, Christian Giardina, Derek Ford, Keri Kodama, Alyssa Anderson (PDKE), Mari-Vaughn Johnson, Heather Kerkering, Patrick Grady (PI-CASC), Elliott Parsons (PI-RISCC), ",
...
CITA <- block_list(fpar(ftext("Suggested Citation", fp_CITA1)),
  fpar(ftext(
    paste0(
      "Longman, R.J., Ford, D.J., Anderson, A., Frazier, A.G, Giardina, C.P (2023). ",
      "Climate Change, Climate Variability and Drought Portfolio ",
      NameF,
      " Pacific Drought knowledge Exchange CCVD Series Version ",
      ver,
      "."
    ),
    fp_CITA2
...
S27_TIT <- block_list(fpar(ftext("Works Cited", FTXTT), fp_p = fp_par(text.align = "center")))

Worksfile <- paste0(I_FOLDER, "Works2.jpg")
Worksimg <- external_img(src = Worksfile ,
  height = 1,
  width = 1)
...
RF100 <-
  paste0(
    "The 100+ year monthly rainfall dataset was drawn from two unique gridded products. We used data from Frazier et al. (2016) ",
    "for the period 1920-1989 and Lucas et al. (2022) for the period 1990-",
    yr,
    ". Given that two unique data sets and methods ",
...
FIG_A1 <- block_list(fpar(ftext(
  paste0(
    "Figure A1: One to one comparison of 23-years (1990-2012) of monthly rainfall ",
    "from two unique datasets for ",
    SNameF,
    ", and associated error metrics; R2, is the coefficient of determination, ",
    "MBE, is the mean bias error, MAE, mean absolute error."
  ),
  fp_Fig
)))
...
DownA <-
  paste0(
    "Two types of downscaling products were used in this analysis. Here we explain some of the nuances between the two. ",
...
TAB3 <- block_list(fpar(ftext(
  paste0(
    ...
    "Magnitude is sum of absolute SPI values during the drought."
  ),
  fp_Fig
)))
